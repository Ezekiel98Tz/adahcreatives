...
export function HomePageEditor() {
  ...
  async function loadData() {
    try {
      ...
      const normalized = {
        ...
        trustedBy: Array.isArray(homeData?.trustedBy) ? homeData.trustedBy : [],
      };
      setData(normalized);
    } catch (e) {
      console.error(e);
  ...
  function addTrustedBy() {
    setData((prev: any) => {
      ...
      return { ...(prev || {}), trustedBy: [...trustedBy, { name: '', logo: { url: '' } }] };
    });
  }
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <form className="space-y-8">
        ...
        <section className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h2 className="text-lg font-medium text-gray-900 mb-4 pb-2 border-b">Visual Stories Section</h2>
          <div className="grid gap-6">
            <div>
        ...
        <section className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-4">
            {(Array.isArray(data.trustedBy) ? data.trustedBy : []).map((item: any, index: number) => {
              ...
              return (
                <div key={`${item?.name || 'company'}-${index}`} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                  <div className="flex items-center justify-between">
